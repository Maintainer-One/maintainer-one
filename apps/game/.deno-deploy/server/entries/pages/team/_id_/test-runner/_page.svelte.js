...
import { M as get, X as escape_html, Y as attr, a as derived, d as stringify, n as attr_class, o as ensure_array_like, r as attr_style } from "../../../../../chunks/dev.js";
	...
	$$renderer.component(($$renderer) => {
		...
		function handleHomeVersionChange() {
			if (selectedHomeVersionId === "scratchpad" && teamId()) {
				const teamDrafts = get(scratchpad)[teamId()];
				teamDrafts && teamDrafts.length > 0 && teamDrafts[0].code;
			} else {
		...
		if (stats()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12 flex-shrink-0"><div class="rounded-2xl border border-white/5 bg-black/20 p-6 backdrop-blur-xl"><div class="text-[9px] font-black uppercase tracking-widest text-[var(--color-brand-primary)] mb-2">Win Rate</div> <div class="text-4xl font-black text-white">${escape_html(stats().winRate.toFixed(1))}%</div></div> <div class="rounded-2xl border border-white/5 bg-black/20 p-6 backdrop-blur-xl"><div class="text-[9px] font-black uppercase tracking-widest text-white/20 mb-2">Avg Score</div> <div class="text-2xl font-black text-white"><span class="text-blue-400">${escape_html(stats().avgScoreA.toFixed(1))}</span> <span class="text-white/10 mx-2">/</span> <span class="text-rose-400">${escape_html(stats().avgScoreB.toFixed(1))}</span></div></div> <div class="rounded-2xl border border-white/5 bg-black/20 p-6 backdrop-blur-xl"><div class="text-[9px] font-black uppercase tracking-widest text-amber-500/60 mb-2">Avg Luck (${escape_html(homeTeam?.name || "Home")})</div> <div${attr_class(`text-2xl font-black ${stringify(stats().avgLuckA >= 0 ? "text-emerald-400" : "text-rose-400")}`)}>${escape_html(stats().avgLuckA > 0 ? "+" : "")}${escape_html(stats().avgLuckA.toFixed(2))}</div></div> <div class="rounded-2xl border border-white/5 bg-black/20 p-6 backdrop-blur-xl"><div class="text-[9px] font-black uppercase tracking-widest text-white/20 mb-2">Samples</div> <div class="text-4xl font-black text-white">${escape_html(batchResults.length)}</div></div></div> <div class="flex-1 min-h-0 rounded-2xl border border-white/5 bg-black/20 backdrop-blur-xl overflow-hidden flex flex-col shadow-2xl"><div class="overflow-y-auto no-scrollbar flex-1 svelte-1fcoz90"><table class="w-full text-left border-collapse"><thead class="sticky top-0 bg-zinc-900/90 backdrop-blur-md z-10 border-b border-white/5"><tr><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20">Seed</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20">Result</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20 text-center">Score</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20 text-center">Luck</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20 text-right">Action</th></tr></thead><tbody class="divide-y divide-white/5"><!--[-->`);

import { G as attr, K as escape_html, a as attr_style, f as stringify, i as attr_class, s as ensure_array_like } from "../../../../chunks/index-server.js";
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (activeTab === "schedule") {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12"><aside class="space-y-6"><h3 class="text-[10px] font-black uppercase tracking-[0.3em] text-white/20">Seasons</h3> <div class="space-y-2"><!--[-->`);
			...
			for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
				...
				const endTime = season.end_date ? new Date(season.end_date).getTime() : startTime + 336 * 60 * 60 * 1e3;
				const isCompleted = nowTime > endTime;
				const isActive = nowTime >= startTime && nowTime <= endTime;
				$$renderer.push(`<div class="group relative"><button${attr_class(`w-full flex items-end justify-between p-4 rounded-xl border transition-all ${stringify(selectedSeasonId === season.id ? "bg-[var(--color-brand-primary)]/10 border-[var(--color-brand-primary)]/30 text-white" : "bg-black/20 border-white/5 text-white/40 hover:bg-black/40")}`)}><div class="flex flex-col items-start"><span class="text-[10px] font-black uppercase tracking-widest">Season ${escape_html(season.season_number)}</span> <span class="text-xs font-bold">${escape_html(season.name)}</span> <div class="flex items-center gap-2 mt-1 opacity-40"><span class="text-[8px] font-black uppercase">${escape_html(new Date(season.start_date).toLocaleDateString())}</span> <span class="text-[8px] font-black opacity-20">→</span> <span class="text-[8px] font-black uppercase">${escape_html(season.end_date ? new Date(season.end_date).toLocaleDateString() : "??")}</span> <span class="text-[8px] font-black ml-1 bg-white/5 px-1 rounded">${escape_html(season.game_slots?.length || season.game_density)} SLOTS</span></div></div> <div${attr_class(`px-2 py-0.5 rounded ${stringify(isActive ? "bg-emerald-500/20 text-emerald-400" : isCompleted ? "bg-white/10 text-white/40" : "bg-black/40 text-[var(--color-brand-primary)]")} text-[8px] font-black uppercase tracking-widest`)}>${escape_html(isActive ? "ACTIVE" : isCompleted ? "COMPLETED" : "UPCOMING")}</div></button> <div class="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all z-10"><a${attr("href", `${stringify(base)}/admin/league/season/new?league=${stringify(selectedLeagueId)}&template=${stringify(season.id)}`)} class="p-1.5 rounded-lg bg-[var(--color-brand-primary)]/10 text-[var(--color-brand-primary)]/40 hover:bg-[var(--color-brand-primary)] hover:text-black transition-all" title="Use as Template"><svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></a> <button class="p-1.5 rounded-lg bg-red-500/10 text-red-500/40 hover:bg-red-500 hover:text-white transition-all" title="Delete Season"><svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div></div>`);
			}
			$$renderer.push(`<!--]--></div></aside> <div class="lg:col-span-2 space-y-8">`);
			if (selectedSeasonId) {
				$$renderer.push("<!--[0-->");
				...
				if (matches.length > 0) {
					...
					$$renderer.push(`<!--]--> <div class="rounded-3xl border border-white/5 bg-black/20 backdrop-blur-xl overflow-hidden shadow-2xl"><table class="w-full text-left"><thead><tr class="border-b border-white/5 bg-white/5"><th class="px-6 py-4 w-12"><input type="checkbox"${attr("checked", matches.length > 0 && selectedMatchIds.size === matches.length, true)} class="accent-[var(--color-brand-primary)] cursor-pointer"/></th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20">Matchup</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20">Scheduled Time</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20">Status</th><th class="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-white/20 text-right">Action</th></tr></thead><tbody class="divide-y divide-white/5"><!--[-->`);
					const each_array_2 = ensure_array_like(matches);
					for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
						let match = each_array_2[$$index_2];
						$$renderer.push(`<tr${attr_class(`hover:bg-white/5 transition-colors ${stringify(selectedMatchIds.has(match.id) ? "bg-white/5" : "")}`)}><td class="px-6 py-4"><input type="checkbox"${attr("checked", selectedMatchIds.has(match.id), true)} class="accent-[var(--color-brand-primary)] cursor-pointer"/></td><td class="px-6 py-4"><div class="flex flex-col gap-1"><div class="flex items-center gap-2 text-xs font-bold text-white"><span${attr_style(`color: ${stringify(match.home_team.color || "white")}`)}>${escape_html(match.home_team.name)}</span> <span class="text-white/10 uppercase text-[9px]">vs</span> <span${attr_style(`color: ${stringify(match.away_team.color || "white")}`)}>${escape_html(match.away_team.name)}</span></div> `);
						if (match.status === "played" || match.status === "simmed" && match.home_score !== null) {
							$$renderer.push("<!--[0-->");
							$$renderer.push(`<div class="text-[10px] font-black text-[var(--color-brand-primary)]">${escape_html(match.home_score)} - ${escape_html(match.away_score)}</div>`);
						} else $$renderer.push("<!--[-1-->");
						$$renderer.push(`<!--]--></div></td><td class="px-6 py-4"><div class="flex items-center gap-2 group"><div class="flex flex-col"><input type="datetime-local"${attr("value", (/* @__PURE__ */ new Date(new Date(match.scheduled_time).getTime() - (/* @__PURE__ */ new Date()).getTimezoneOffset() * 6e4)).toISOString().slice(0, 16))} class="bg-transparent border-none text-[10px] font-bold text-white/40 focus:text-white focus:outline-none cursor-pointer p-0"/> `);
						if (match.code_lock_time) {
							$$renderer.push("<!--[0-->");
							$$renderer.push(`<span class="text-[8px] font-black text-white/20 uppercase mt-1">Locks: ${escape_html(new Date(match.code_lock_time).toLocaleTimeString([], {
								hour: "2-digit",
								minute: "2-digit"
							}))}</span>`);
						} else $$renderer.push("<!--[-1-->");
						$$renderer.push(`<!--]--></div> <button class="text-white/20 hover:text-white transition-colors opacity-0 group-hover:opacity-100" title="Copy Time"><svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button> `);
						$$renderer.push("<!--[-1-->");
						$$renderer.push(`<!--]--></div></td><td class="px-6 py-4"><span${attr_class(`text-[9px] font-black uppercase tracking-widest ${stringify(match.status === "played" ? "text-emerald-400" : match.status === "simmed" ? "text-blue-400" : match.status === "scheduled" ? "text-amber-400" : "text-white/40")}`)}>${escape_html(match.status)}</span></td><td class="px-6 py-4 text-right">`);
						if (match.status === "pending" || match.status === "scheduled") {
							$$renderer.push("<!--[0-->");
							$$renderer.push(`<button${attr("disabled", isSimulatingMatch === match.id, true)} class="text-[9px] font-black uppercase tracking-widest text-[var(--color-brand-primary)] hover:underline disabled:opacity-50">${escape_html(isSimulatingMatch === match.id ? "Simulating..." : "Run Sim Now")}</button>`);
						} else {
							$$renderer.push("<!--[-1-->");
							$$renderer.push(`<a${attr("href", `${stringify(base)}/match/${stringify(match.id)}`)} class="text-[9px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-colors">View Replay</a>`);
						}
						$$renderer.push(`<!--]--></td></tr>`);
					}
					...
					$$renderer.push("<!--[-1-->");
					$$renderer.push(`<div class="py-24 text-center border-2 border-dashed border-white/5 rounded-3xl"><p class="text-sm font-bold text-white/20 uppercase tracking-widest">No matches found for this season</p></div>`);
				}
				$$renderer.push(`<!--]-->`);
			} else {
				$$renderer.push("<!--[-1-->");

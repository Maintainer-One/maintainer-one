...
function MatchPreview($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<div class="flex-1 flex flex-col items-center justify-center p-8 overflow-y-auto no-scrollbar svelte-13nzxi7"><div class="w-full max-w-4xl space-y-12"><div class="text-center space-y-6"><div class="inline-flex items-center gap-2 rounded-full border border-[var(--color-brand-primary)]/30 bg-[var(--color-brand-primary)]/10 px-4 py-1.5 text-[10px] font-black tracking-[0.2em] text-[var(--color-brand-secondary)] uppercase"><span class="h-2 w-2 animate-pulse rounded-full bg-[var(--color-brand-primary)]"></span> Match Preview</div> <div class="flex items-center justify-center gap-12 md:gap-24"><div class="flex flex-col items-center gap-4"><div class="h-24 w-24 rounded-3xl flex items-center justify-center text-4xl font-black border-2 shadow-2xl"${attr_style(`border-color: ${stringify(match.home_team.color)}44; background-color: ${stringify(match.home_team.color)}22; color: ${stringify(match.home_team.color)}`)}>${escape_html(match.home_team.name[0])}</div> <h2 class="text-2xl font-black text-white tracking-tighter">${escape_html(match.home_team.name)}</h2></div> <div class="flex flex-col items-center gap-2"><span class="text-6xl font-black text-white/5 tracking-tighter uppercase italic">VS</span> <div class="text-2xl font-mono font-black text-[var(--color-brand-primary)] drop-shadow-[0_0_20px_rgba(16,185,129,0.3)]">${escape_html("")}</div></div> <div class="flex flex-col items-center gap-4"><div class="h-24 w-24 rounded-3xl flex items-center justify-center text-4xl font-black border-2 shadow-2xl"${attr_style(`border-color: ${stringify(match.away_team.color)}44; background-color: ${stringify(match.away_team.color)}22; color: ${stringify(match.away_team.color)}`)}>${escape_html(match.away_team.name[0])}</div> <h2 class="text-2xl font-black text-white tracking-tighter">${escape_html(match.away_team.name)}</h2></div></div></div> `);
		$$renderer.push("<!--[-1-->");
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const isCompleted = derived(() => {
			if (!matchData || states.length === 0) return false;
			return currentTime >= new Date(matchData.scheduled_time).getTime() + (states.length - 1) * playSpeed;
		});
		async function loadMatch(matchId) {
			...
			matchData = match;
			isPreview = /* @__PURE__ */ new Date() < new Date(match.scheduled_time);
			if (isPreview) {
				isSimulating = false;
				return;
			}
			...
			states = [createInitialState(Number(match.seed), match.seasons?.protocol_version || match.leagues.protocol_version, config, {
				A: match.home_team,
				B: match.away_team
			})];
			currentTick = 0;
		...
		$$renderer.push(`<div class="flex h-screen w-full overflow-hidden bg-[var(--color-background-dark)] text-[var(--color-brand-secondary)]/90 font-sans selection:bg-[var(--color-brand-primary)]/30"><main class="flex flex-1 flex-col p-6 lg:p-10 relative overflow-hidden"><header class="mb-8 flex items-center justify-between"><div class="flex items-center gap-6"><a${attr("href", returnToUrl())} class="group flex h-10 w-10 items-center justify-center rounded-xl border border-white/5 bg-black/20 text-white/20 transition-all hover:text-[var(--color-brand-primary)] shadow-lg backdrop-blur-md"><svg class="h-5 w-5 transition-transform group-hover:-translate-x-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg></a> <div class="flex items-center gap-4 bg-black/40 rounded-2xl border border-white/5 px-6 py-3 backdrop-blur-3xl shadow-2xl">`);
		if (matchData) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="text-right"><div class="text-[9px] font-black uppercase tracking-widest leading-none mb-1 opacity-40">Home</div> <div class="text-sm font-black leading-none"${attr_style(`color: ${stringify(matchData.home_team.color)}`)}>${escape_html(matchData.home_team.name)}</div></div> <div class="px-3 text-[10px] font-black text-white/10 uppercase tracking-widest">vs</div> <div class="text-left"><div class="text-[9px] font-black uppercase tracking-widest leading-none mb-1 opacity-40">Away</div> <div class="text-sm font-black leading-none"${attr_style(`color: ${stringify(matchData.away_team.color)}`)}>${escape_html(matchData.away_team.name)}</div></div>`);
		} else {
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="h-8 w-40 bg-white/5 rounded-lg animate-pulse"></div>`);
		}
		$$renderer.push(`<!--]--></div> `);
		if (isLive()) {
			$$renderer.push("<!--[0-->");
		...
		if (isPreview) {
			...
			$$renderer.push("<!--[1-->");
			$$renderer.push(`<div class="flex-1 flex flex-col items-center justify-center min-h-0"><div class="flex-1 w-full max-w-4xl min-h-0 p-4">`);
			ReplayGrid($$renderer, {
				state: currentState(),
				showControlMap,
				playSpeed
			...
			$$renderer.push("<!--[2-->");
			$$renderer.push(`<div class="flex flex-1 items-center justify-center italic text-white/20 font-black uppercase tracking-widest text-sm animate-pulse">Broadcasting from Arena...</div>`);
		} else $$renderer.push("<!--[-1-->");
		...
		if (currentState()) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="space-y-12"><section><h3 class="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] mb-6">Match Score</h3> <div class="grid grid-cols-2 gap-4"><div class="rounded-2xl border bg-black/20 p-6 text-center shadow-xl"${attr_style(`border-color: ${stringify(currentState().teams.A.color)}11`)}><div class="text-[8px] font-black uppercase tracking-widest mb-1"${attr_style(`color: ${stringify(currentState().teams.A.color)}`)}>${escape_html(currentState().teams.A.name)}</div> <div class="text-5xl font-black text-white leading-none">${escape_html(currentState().teams.A.score)}</div></div> <div class="rounded-2xl border bg-black/20 p-6 text-center shadow-xl"${attr_style(`border-color: ${stringify(currentState().teams.B.color)}11`)}><div class="text-[8px] font-black uppercase tracking-widest mb-1"${attr_style(`color: ${stringify(currentState().teams.B.color)}`)}>${escape_html(currentState().teams.B.name)}</div> <div class="text-5xl font-black text-white leading-none">${escape_html(currentState().teams.B.score)}</div></div></div></section> <section><h3 class="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] mb-6">Fortune / Luck (xS)</h3> <div class="space-y-4"><!--[-->`);

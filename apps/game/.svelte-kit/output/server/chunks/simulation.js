...
async function runSimulation(match) {
	...
	const initialState = createInitialState(Number(match.seed), match.seasons?.protocol_version || match.leagues.protocol_version, match.seasons?.protocol_config ?? match.leagues.protocol_config, {
		A: match.home_team,
		B: match.away_team
	});
